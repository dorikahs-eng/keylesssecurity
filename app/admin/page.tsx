...
export default function AdminPage() {
  ...
  const filtered = orders.filter(o => {
    const matchFilter =
      filter === 'all' ||
      o.status === filter ||
      (filter === 'new-homeowner' && o.type === 'new-homeowner') ||
      (filter === 'existing' && o.type === 'existing');
      ...
      <div style={{ minHeight: '100vh', background: '#0A1628', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
        <div style={{ width: '100%', maxWidth: '360px' }}>
          ...
          <form onSubmit={login} style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
            <div>
              <label style={{ display: 'block', fontFamily: 'var(--font-syne)', fontSize: '0.78rem', fontWeight: 600, color: 'rgba(255,255,255,0.5)', marginBottom: '0.35rem' }}>Admin Password</label>
              ...
              {pwError && <p style={{ fontSize: '0.75rem', color: '#EF4444', marginTop: '0.25rem' }}>{pwError}</p>}
            </div>
            <button type="submit" style={{ background: '#FF5500', color: 'white', fontFamily: 'var(--font-syne)', fontWeight: 700, padding: '0.875rem', borderRadius: '10px', border: 'none', cursor: 'pointer', fontSize: '0.95rem' }}>Sign In</button>
          </form>
    ...
    <div style={{ minHeight: '100vh', background: '#0A1628' }}>
      <div style={{ background: '#0d1e35', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '0.875rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          ...
          {lastRefresh && (
            <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.25)', fontFamily: 'var(--font-jakarta)' }}>
              Last updated {lastRefresh.toLocaleTimeString()}
            </span>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <button onClick={loadOrders} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', cursor: 'pointer', fontFamily: 'var(--font-syne)', fontWeight: 600, fontSize: '0.8rem', padding: '0.4rem 0.875rem', borderRadius: '7px' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '1.5rem 1rem 4rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
          {[
            ...
            { icon: DollarSign, label: 'Revenue', value: `$${totalRevenue.toLocaleString()}`, color: '#22C55E' },
            { icon: DoorOpen, label: 'Total Doors', value: totalDoors, color: '#3B82F6' },
            { icon: Clock, label: 'Pending', value: orders.filter(o => o.status === 'pending').length, color: '#F59E0B' },
        ...
        {loading ? (
          <div style={{ ...box, textAlign: 'center', padding: '3rem' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.3)', fontFamily: 'var(--font-syne)', fontSize: '0.875rem' }}>Loading all orders...</p>
          </div>
        ) : (
              ...
              {filtered.length === 0 ? (
                ...
                return (
                  <div key={order.id} onClick={() => setSelected(isSelected ? null : order)}
                    style={{ ...box, cursor: 'pointer', border: isSelected ? '1px solid rgba(255,85,0,0.4)' : '1px solid rgba(255,255,255,0.08)' }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', fontSize: '0.75rem', color: 'rgba(255,255,255,0.35)' }}>
                          ...
                          {(order.scheduledDate || order.scheduledAt) && <span style={{ color: '#60A5FA', fontFamily: 'var(--font-jakarta)', display: 'flex', alignItems: 'center', gap: '0.25rem' }}><Calendar size={10}/>{order.scheduledDate || 'Scheduled'}</span>}
                        </div>
                      </div>
            ...
            {selected && (
              <div style={{ background: '#0d1e35', border: '1px solid rgba(255,85,0,0.25)', borderRadius: '14px', padding: '1.25rem', position: 'sticky', top: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  ...
                  <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', fontSize: '1.1rem' }}>×</button>
                </div>
                ...
                {(selected.scheduledDate || selected.scheduledAt) && (
                  <div style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '8px', padding: '0.75rem', marginBottom: '1rem' }}>
                    ...
                    <p style={{ fontFamily: 'var(--font-syne)', fontWeight: 600, color: 'white', fontSize: '0.875rem' }}>
                      {selected.scheduledDate || 'Via Cal.com'}{selected.scheduledTime && ` at ${selected.scheduledTime}`}
                    </p>
                  </div>
                )}
